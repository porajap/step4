...
public class Cost_2 extends Window{
	String xUser;
	String Dosing;
	...
	public void onSetting() throws Exception{
		session.setAttribute("Page", "Cost_2.zul");
	...
	private void onDisplay() throws Exception{
		...
		try{	
			...
			String xSql = "SELECT cost_dosing.id,cost_dosing.xName,cost_dosing.Qty FROM cost_dosing WHERE cost_dosing.username = '1' ORDER BY cost_dosing.id ASC LIMIT 8";
			rs = sqlsel.getReSultSQL( xSql );
			int n=0;
			while(rs.next()){
				id[n] = rs.getInt("id");
