...
public class Cost_5 extends Window{
	String xUser;
	String Dosing;
	...
	public void onCreate() throws Exception {
		...
		xB17 = ((Decimalbox) getFellow("xB17"));
		xB18 = ((Decimalbox) getFellow("xB18"));
		xB19 = ((Decimalbox) getFellow("xB19"));
		xB20 = ((Decimalbox) getFellow("xB20"));
		...
		xS17 = ((Decimalbox) getFellow("xS17"));
		xS18 = ((Decimalbox) getFellow("xS18"));
		xS19 = ((Decimalbox) getFellow("xS19"));
		xS20 = ((Decimalbox) getFellow("xS20"));
	}
	
	public void onSetting() throws Exception{
		session.setAttribute("Page", "Cost_5.zul");
	...
	public void onCal() throws Exception{
        ...
        try{ BBBBB01 = Number.unCommaReturnDouble( xB17.getValue()+"" );}catch(Exception e){};
        try{ BBBBB02 = Number.unCommaReturnDouble( xB18.getValue()+"" );}catch(Exception e){};
        try{ BBBBB03 = Number.unCommaReturnDouble( xB19.getValue()+"" );}catch(Exception e){};
        try{ BBBBB04 = Number.unCommaReturnDouble( xB20.getValue()+"" );}catch(Exception e){};
		...
		try{	
			...
			if(BBBBB01>0)BBBBB99 = BBBBB02/(BBBBB01*1000);

			String xSql = "SELECT cost_dosing.id,cost_dosing.xName,cost_dosing.Qty FROM cost_dosing WHERE cost_dosing.username = '"+xUser+"' ORDER BY cost_dosing.id ASC LIMIT 20";
			...
			if( (BBBBB01>0) && (BBBBB02>0) && (BBBBB03>0) && (BBBBB04>0)){
				CCCCC01 = (Qty[2]*Qty[3]);
				CCCCC02 = BBBBB03*BBBBB99;
				CCCCC03 = Qty[0]*(BBBBB04/60)*Qty[1];
				CCCCC04 = CCCCC01+CCCCC02+CCCCC03;
				SumC += CCCCC04;
			}
	...
	private void onDisplay() throws Exception{
		...
		try{	
			...
			String xSql = "SELECT cost_dosing.id,cost_dosing.xName,cost_dosing.Qty FROM cost_dosing WHERE cost_dosing.username = '1' ORDER BY cost_dosing.id ASC LIMIT 20";
			rs = sqlsel.getReSultSQL( xSql );
			int n=0;
			while(rs.next()){
				id[n] = rs.getInt("id");
